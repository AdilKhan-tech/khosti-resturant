...
import {
  getQrAttributionReportRoute,
  getQrOrderReportRoute,
} from "@/utils/apiRoutes";
import useAxiosConfig from "@/hooks/useAxiosConfig";
import { ToastContainer, toast } from "react-toastify";
import axios from "axios";

function QrAttributionReportPage() {
  ...
  useEffect(() => {
    if (!token) return;
    axios
      .get(getQrAttributionReportRoute)
      .catch(() => axios.get(getQrOrderReportRoute))
    ...
    <>
      <section className="mt-3">
        <div className="d-flex justify-content-between mb-3 align-items-center flex-wrap gap-2">
          <p className="pagetitle mb-0 fnt-color">QR Attribution</p>
          <Link
...
export default QrAttributionReportPage;
