...
import AddTag from "@/components/dashboard/product/AddTag";
import { useEffect, useState } from "react";
import { getTagsRoute, deleteTagByIdRoute } from "@/utils/apiRoutes";
...
export default function productTags() {
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      const response = await axios.get(getTagsRoute, { params });
  ...
  const handleDelete = async (tagId) => {
    try {
      const response = await axios.delete(deleteTagByIdRoute(tagId));
  ...
  const showDeleteConfirmation = async (tagId) => {
